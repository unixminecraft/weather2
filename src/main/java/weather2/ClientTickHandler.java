...
public class ClientTickHandler {
	...
	private static SceneEnhancer sceneEnhancer;
	private static FoliageEnhancerShader foliageEnhancer;
	...
	public void onTickInGame() {
		...
		if(world != null) {
			...
			// TODO: split logic up a bit better for this, if this is set to false mid sandstorm, fog is stuck
			// on,
			...
			if(diff > 10) {
				...
				smoothAngle = MathHelper.wrapDegrees(smoothAngle);
				...
				if(Math.abs(bestMove) < 180) {
					...
					if(smoothAngleAdj * 2 > windSpeed) {
						if(bestMove > 0) {
							smoothAngleRotationalVelAccel -= smoothAngleAdj;
							if(prevDir < 0) {
	...
	private static void resetClientWeather() {
